...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick }) {
    ...
    return (
        ...
        >
            {/* Bot Avatar */}
            {isBot && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#ccf5e5] to-[#b3f0d8] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#7ee8c0]/50 overflow-hidden p-0.5">
                    {!logoErr ? (
                        <img src="https://www.google.com/s2/favicons?domain=www.zdorovo-dent.com.ua&sz=32" alt="" className="w-full h-full object-contain rounded-sm" onError={() => setLogoErr(true)} />
                    ) : (
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ...
                              : 'bg-gradient-to-br from-[#00d084] to-[#00b070] text-white rounded-br-md shadow-sm shadow-[#00d084]/20'
                    }`}>
                        <div className="max-w-none [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className={`underline decoration-1 underline-offset-2 transition-colors ${isBot ? 'text-[#00b070] hover:text-[#009960]' : 'text-white/90 hover:text-white'}`}>{children}</a>
                                    ),
                                    strong: ({ children }) => (
