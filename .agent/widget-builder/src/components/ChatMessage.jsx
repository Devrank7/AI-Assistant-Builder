...
function BotAvatar() {
    ...
    if (ba && ba.length <= 3) {
        return (
            <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#fde8e9] to-[#f5a0a4] flex items-center justify-center text-[11px] font-semibold border border-[#f4c0c2]/50 shadow-sm">
                {ba}
            </span>
        );
    }
    return (
        <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#fde8e9] to-[#f5a0a4] flex items-center justify-center border border-[#f4c0c2]/50 shadow-sm">
...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick, onSpeak, isSpeaking, prevSender, nextSender }) {
    ...
    return (
        ...
        >
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${tailClass} ${
                        ...
                        <div className="max-w-none msg-text [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className="underline decoration-1 underline-offset-2 transition-colors text-[#ED1C24] hover:text-[#c81820]">{children}</a>
                                    ),
                                    strong: ({ children }) => (
                                    ...
                                    h1: ({ children }) => <h1 className="text-base font-semibold mt-3 mb-1 text-gray-900">{children}</h1>,
                                    h2: ({ children }) => <h2 className="text-sm font-semibold mt-2 mb-1 text-gray-900">{children}</h2>,
                                    h3: ({ children }) => <h3 className="text-sm font-medium mt-2 mb-0.5 text-gray-900">{children}</h3>,
                                    code: ({ children, className: cn }) => cn
                                        ? <code className={cn}>{children}</code>
                                        : <code className="bg-gray-100 px-1 py-0.5 rounded text-[11px] font-mono">{children}</code>,
                                    pre: ({ children }) => <pre className="bg-gray-100 p-2 rounded-lg overflow-x-auto text-[11px] my-2 font-mono">{children}</pre>,
                                    blockquote: ({ children }) => <blockquote className="border-l-2 border-[#ED1C24] pl-2 italic opacity-80 my-1">{children}</blockquote>,
                                }}
                            >
                                {content}
                            </ReactMarkdown>
                ...
                {showMeta && (
                    <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                        ...
                        {isBot && !isError && content && (
                            <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-[#ED1C24] transition-all duration-200" aria-label="Copy">
                                {copied ? <Check size={11} className="text-[#ED1C24]" /> : <Copy size={11} />}
                            </button>
                        )}
                        {isBot && !isError && content && onSpeak && (
                            <button onClick={() => onSpeak(content)}
                                className={`p-0.5 transition-all duration-200 ${isSpeaking ? 'text-[#ED1C24] opacity-100' : 'opacity-0 group-hover:opacity-100 text-gray-300 hover:text-[#ED1C24]'}`}
            ...
            {!isBot && (showAvatar ? (
                <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-[#ED1C24] text-white flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
