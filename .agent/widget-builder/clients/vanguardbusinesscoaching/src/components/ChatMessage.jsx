...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick, onSpeak, isSpeaking }) {
    ...
    return (
        ...
        >
            {/* Bot Avatar */}
            {isBot && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#ebfaeb] to-[#c3efc3] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#afeaaf]/50">
                ...
                {content && (
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ? 'bg-[#091d1b] text-[#e2e8f0] border border-[#11312a] shadow-sm rounded-bl-md'
                              : 'bg-gradient-to-r from-[#37ca37] to-[#188bf6] text-white rounded-br-md shadow-sm'
                    }`}>
            ...
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#ebfaeb] to-[#c3efc3] border border-[#afeaaf]/50 text-[#37ca37] flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
