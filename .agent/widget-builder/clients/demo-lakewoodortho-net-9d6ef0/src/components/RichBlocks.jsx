            ...
            <div className="p-3 space-y-1.5">
                ...
                {card.button && (
                    <button onClick={() => onAction?.(card.button.url, card.button.label)}
                        className="w-full mt-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all bg-[#e6f4f9] text-[#00799f] hover:bg-[#b3dfed] flex items-center justify-center gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
            {buttons.map((btn, i) => (
                <button key={i} onClick={() => onAction?.(btn.url, btn.label)}
                    className="px-2.5 py-1.5 rounded-xl border text-[11px] font-medium transition-all cursor-pointer border-[#99d4e7] bg-[#e6f4f9] text-[#00799f] hover:bg-[#b3dfed] hover:border-[#66bfda]">
                    {btn.label}
                </button>
            ))}
        </div>
        ...
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    ...
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#e6f4f9] transition-all bg-white border-gray-200 text-gray-800 placeholder-gray-400 focus:border-[#66bfda]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#0094C2] hover:bg-[#00799f] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
