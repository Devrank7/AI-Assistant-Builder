            ...
            <div className="p-3 space-y-1.5">
                ...
                {card.button && (
                    <button onClick={() => onAction?.(card.button.url, card.button.label)}
                        className="w-full mt-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all bg-[#e6f3f4] text-[#006b75] hover:bg-[#b3dadd] flex items-center justify-center gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
            {buttons.map((btn, i) => (
                <button key={i} onClick={() => onAction?.(btn.url, btn.label)}
                    className="px-2.5 py-1.5 rounded-xl border text-[11px] font-medium transition-all cursor-pointer border-[#99cdd2] bg-[#e6f3f4] text-[#006b75] hover:bg-[#b3dadd] hover:border-[#66b5bc]">
                    {btn.label}
                </button>
            ))}
        </div>
...
function LeadForm({ fields, submitLabel, onSubmit }) {
    ...
    return (
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#00838F] hover:bg-[#006b75] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
