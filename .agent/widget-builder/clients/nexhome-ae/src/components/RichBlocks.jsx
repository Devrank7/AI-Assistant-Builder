        ...
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    ...
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#201b0c] transition-all bg-[#0f1720] border-[#1e2d3d] text-[#e2e8f0] placeholder-[#475569] focus:border-[#e6d195]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#D6B24F] hover:bg-[#ab8e3f] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
