...
function Card({ card, onAction }) {
    return (
        <div className="flex-shrink-0 w-[200px] rounded-2xl border overflow-hidden bg-[#111927] border-[#1e2d3d] shadow-sm">
            {card.image && (
        ...
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    ...
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#001f19] transition-all bg-[#0f1720] border-[#1e2d3d] text-[#e2e8f0] placeholder-[#475569] focus:border-[#66e2ca]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#00CEA6] hover:bg-[#00a585] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
