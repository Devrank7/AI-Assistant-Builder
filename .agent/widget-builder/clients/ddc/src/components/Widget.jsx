...
export function Widget({ config }) {
    ...
    return (
        <div className={`fixed z-50 flex flex-col gap-4 antialiased ${positionClasses}`} style={{ fontFamily: "\'Montserrat\', Helvetica, Arial, sans-serif", ...dragStyle }}>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="relative w-[85vw] max-w-[370px] h-[60vh] max-h-[540px] sm:w-[370px] sm:h-[540px] rounded-3xl overflow-hidden flex flex-col bg-[#0e0e0e] shadow-2xl shadow-black/40 border border-[#2e2e2e]"
                        role="dialog"
                        aria-label="Chat widget"
                    >
                        {/* HEADER */}
                        <div className="relative px-5 py-4 flex items-center justify-between overflow-hidden ">
                            <div className="absolute inset-0 bg-gradient-to-br from-[#1a1a1a] via-[#252525] to-[#333333]" />

                            <div className="relative flex items-center gap-3">
                                <div className="relative">
                                    <div className="w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-inner shadow-white/10 overflow-hidden p-1.5">
                                        {!headerLogoErr ? (
                                            <img src="https://www.google.com/s2/favicons?domain=ddc.clinic&sz=64" alt="" className="w-full h-full object-contain rounded" crossOrigin="anonymous" onError={() => setHeaderLogoErr(true)} />
                                        ) : (
                                            <Sparkles size={18} className="text-white" />
                                        )}
                                    </div>
                                    {!isOffline && (
                                        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#8a8a8a] border-[2.5px] border-[#333333] shadow-sm" />
                                    )}
                                </div>
                                <div>
                        ...
                        {/* MESSAGES */}
                        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 scrollbar-hide bg-gradient-to-b from-[#0e0e0e] to-[#1a1a1a]" aria-live="polite">
                            ...
                            {isTyping && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 items-center ml-10 py-2">
                                    {[0, 1, 2].map((i) => (
                                        <motion.span key={i} className="w-2 h-2 bg-[#565756] rounded-full"
                        ...
                        <AnimatePresence>
                            {selectedImage && (
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 overflow-hidden bg-[#0e0e0e] border-[#2e2e2e]">
                                    <div className="relative inline-block my-2.5">
                                        <img src={selectedImage.previewUrl} alt="" className="h-16 w-auto rounded-xl border border-[#2e2e2e] object-cover shadow-sm" />
                        ...
                        {/* INPUT */}
                        <div className="px-4 py-3.5 border-t border-[#2e2e2e] bg-[#0e0e0e] space-y-2.5">
                            ...
                            <form onSubmit={handleSubmit} className="flex items-end gap-2">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#565756] bg-[#1a1a1a] text-[#a0a0a0] shadow-sm' : 'border-[#2e2e2e] text-[#808080] hover:text-[#a0a0a0] hover:border-[#444444] hover:bg-[#1a1a1a]'}`}
                                ...
                                <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                                    ...
                                    rows={1}
                                    className="flex-1 min-w-0 bg-[#161616] text-[#e0e0e0] placeholder-[#808080] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#2e2e2e] focus:outline-none focus:border-[#565756] focus:ring-2 focus:ring-[#2a2a2a] focus:bg-[#1e1e1e] transition-all resize-none text-[13.5px] leading-relaxed"
                                    style={{ maxHeight: '100px' }}
                                />
                                <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                                    className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#565756] text-white flex items-center justify-center hover:bg-[#6a6b6a] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#565756]/25">
                                    <Send size={16} />
                                </button>
                            </form>
                        </div>
            ...
            {/* TOGGLE BUTTON */}
            <motion.button
                whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(50, 50, 50, 0.35)' }}
                ...
                className="w-[56px] h-[56px] rounded-xl flex items-center justify-center text-white shadow-lg shadow-[#000000]/30 bg-gradient-to-br from-[#000000] via-[#1a1a1a] to-[#2a2a2a] border border-white/10"
