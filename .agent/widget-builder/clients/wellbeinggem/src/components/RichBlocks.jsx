...
function Card({ card, onAction }) {
    return (
        <div className="flex-shrink-0 w-[170px] sm:w-[200px] rounded-2xl border overflow-hidden bg-[#141d2b] border-[#203140] shadow-sm">
            {card.image && (
            ...
            <div className="p-3 space-y-1.5">
                ...
                {card.button && (
                    <button onClick={() => onAction?.(card.button.url, card.button.label)}
                        className="w-full mt-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all bg-[#0b1621] text-[#e2e8f0] hover:bg-[#162232] flex items-center justify-center gap-1">
        ...
        <div className="relative group">
            ...
            {canScrollLeft && (
                <button onClick={() => scroll(-1)}
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-[#141d2b]/90 border-[#203140] hover:bg-[#141d2b] shadow-md border flex items-center justify-center transition-all z-10">
                    <ChevronLeft size={14} className="text-[#64748b]" />
                </button>
            )}
            {canScrollRight && (
                <button onClick={() => scroll(1)}
                    className="absolute right-0 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-[#141d2b]/90 border-[#203140] hover:bg-[#141d2b] shadow-md border flex items-center justify-center transition-all z-10">
        ...
        <div className="flex flex-wrap gap-1.5">
            {buttons.map((btn, i) => (
                <button key={i} onClick={() => onAction?.(btn.url, btn.label)}
                    className="px-2.5 py-1.5 rounded-xl border text-[11px] font-medium transition-all cursor-pointer border-[#203140] bg-[#141d2b] text-[#e2e8f0] hover:bg-[#0b1621]">
                    {btn.label}
                </button>
            ))}
        </div>
...
function LeadForm({ fields, submitLabel, onSubmit }) {
    ...
    if (submitted) {
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="rounded-2xl border p-3 text-center bg-[#141d2b] border-[#203140]">
    ...
    return (
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            onSubmit={handleSubmit} className="rounded-2xl border p-3 space-y-2 bg-[#141d2b] border-[#203140]">
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    placeholder={f.label}
                    value={values[f.key] || ''}
                    onChange={(e) => setValues(prev => ({ ...prev, [f.key]: e.target.value }))}
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#f8fafe] transition-all bg-[#0b1621] border-[#203140] text-[#e2e8f0] placeholder-[#475569] focus:border-[#d5dff9]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#b9caf5] hover:bg-[#94a2c4] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
