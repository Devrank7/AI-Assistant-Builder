...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick }) {
    ...
    return (
        ...
        >
            {/* Bot Avatar */}
            {isBot && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#d9f0fc] to-[#bee6f9] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#7dcaf2]/50 overflow-hidden p-0.5">
                    {!logoErr ? (
                        <img src="https://www.google.com/s2/favicons?domain=med-deo.com.ua&sz=32" alt="" className="w-full h-full object-contain rounded-sm" onError={() => setLogoErr(true)} />
                    ) : (
                        <Sparkles size={13} className="text-[#0581cc]" />
                    )}
                </div>
            )}
            ...
            <div className="flex flex-col max-w-[78%]">
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ...
                              : 'bg-gradient-to-r from-[#ecf7fd] to-[#d9f0fc] text-gray-700 border border-[#7dcaf2]/50 rounded-br-md shadow-sm'
                    }`}>
                        <div className="max-w-none [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className="underline decoration-1 underline-offset-2 transition-colors text-[#0581cc] hover:text-[#0470b5]">{children}</a>
                                    ),
                                    strong: ({ children }) => (
                ...
                <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                    ...
                    {isBot && !isError && content && (
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-[#0693e3] transition-all duration-200" aria-label="Copy">
                            {copied ? <Check size={11} className="text-[#0693e3]" /> : <Copy size={11} />}
                        </button>
                    )}
                    {isError && onRetry && (
            ...
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#d9f0fc] to-[#bee6f9] border border-[#7dcaf2]/50 text-[#0581cc] flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
