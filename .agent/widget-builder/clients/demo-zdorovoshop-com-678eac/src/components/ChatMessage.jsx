...
function BotAvatar() {
    ...
    if (ba && ba.length <= 3) {
        return (
            <span className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#e6f5f0] to-[#b4e0d2] flex items-center justify-center text-[11px] font-semibold border border-[#9bd5c3]/50 shadow-sm">
                {ba}
            </span>
        );
    }
    return (
        <span className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#e6f5f0] to-[#b4e0d2] flex items-center justify-center border border-[#9bd5c3]/50 shadow-sm">
...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick, onSpeak, isSpeaking, prevSender, nextSender }) {
    ...
    return (
        ...
        >
                        ...
                        <div className="max-w-none msg-text [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    ...
                                    blockquote: ({ children }) => <blockquote className="border-l-2 border-[#059669] pl-2 italic opacity-80 my-1">{children}</blockquote>,
                                }}
                            >
                                {content}
                            </ReactMarkdown>
                    ...
                    <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                        ...
                        {isBot && !isError && content && onSpeak && (
                            <button onClick={() => onSpeak(content)}
                                className={`p-0.5 transition-all duration-200 ${isSpeaking ? 'text-[#059669] opacity-100' : 'opacity-0 group-hover:opacity-100 text-gray-300 hover:text-[#059669]'}`}
            ...
            {!isBot && (showAvatar ? (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-[#059669] text-white flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
