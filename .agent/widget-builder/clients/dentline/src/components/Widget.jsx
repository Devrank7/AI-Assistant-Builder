    ...
    return (
        <div className={`fixed z-50 flex flex-col gap-4 antialiased ${positionClasses}`} style={{ fontFamily: "-apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, sans-serif", ...dragStyle }}>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="relative w-[85vw] max-w-[350px] h-[60vh] max-h-[500px] sm:w-[350px] sm:h-[500px] rounded-3xl overflow-hidden flex flex-col bg-white shadow-2xl shadow-black/15 border border-gray-100"
                        role="dialog"
                        aria-label="Chat widget"
                    >
                        {/* HEADER */}
                        <div className="relative px-5 py-3.5 flex items-center justify-between overflow-hidden ">
                            <div className="absolute inset-0 bg-gradient-to-br from-[#2d8659] via-[#339963] to-[#3a9e6b]" />
                            ...
                            <div className="relative flex items-center gap-3">
                                <div className="relative">
                                    <div className="w-10 h-10 rounded-lg bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-inner shadow-white/10 overflow-hidden p-1.5">
                                        {!headerLogoErr ? (
                                            <img src="https://www.google.com/s2/favicons?domain=dentline.com.ua&sz=64" alt="" className="w-full h-full object-contain rounded" crossOrigin="anonymous" onError={() => setHeaderLogoErr(true)} />
                                        ) : (
                                            <Sparkles size={18} className="text-white" />
                                        )}
                                    </div>
                                    {!isOffline && (
                                        <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#8cc9a6] border-[2.5px] border-[#2d8659] shadow-sm" />
                                    )}
                                </div>
                                <div>
                        ...
                        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 scrollbar-hide bg-gradient-to-b from-gray-50/80 to-white" aria-live="polite">
                            ...
                            {isTyping && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 items-center ml-10 py-2">
                                    {[0, 1, 2].map((i) => (
                                        <motion.span key={i} className="w-2 h-2 bg-[#4aad78] rounded-full"
                        ...
                        <div className="px-4 py-3.5 border-t border-gray-100 bg-white space-y-2.5">
                            ...
                            <form onSubmit={handleSubmit} className="flex items-end gap-2">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#8cc9a6] bg-[#edf7f1] text-[#2d8659] shadow-sm' : 'border-gray-200 text-gray-400 hover:text-[#2d8659] hover:border-[#8cc9a6] hover:bg-[#edf7f1]'}`}
                                ...
                                <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                                    ...
                                    rows={1}
                                    className="flex-1 min-w-0 bg-gray-50/80 text-gray-800 placeholder-gray-400 rounded-xl py-2.5 pl-3.5 pr-3.5 border border-gray-200 focus:outline-none focus:border-[#4aad78] focus:ring-2 focus:ring-[#d4eede] focus:bg-white transition-all resize-none text-[13.5px] leading-relaxed"
                                    style={{ maxHeight: '100px' }}
                                />
                                <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                                    className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#2d8659] text-white flex items-center justify-center hover:bg-[#246e49] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#2d8659]/25">
                                    <Send size={16} />
                                </button>
                            </form>
                        </div>
            ...
            <motion.button
                ...
                className="w-[50px] h-[50px] rounded-xl flex items-center justify-center text-white shadow-lg shadow-[#2d8659]/30 bg-gradient-to-br from-[#2d8659] via-[#339963] to-[#3a9e6b] border border-white/10"
