            ...
            <div className="p-3 space-y-1.5">
                ...
                {card.button && (
                    <button onClick={() => onAction?.(card.button.url, card.button.label)}
                        className="w-full mt-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all bg-[#f4f8fd] text-[#1564b3] hover:bg-[#e8f1fb] flex items-center justify-center gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
            {buttons.map((btn, i) => (
                <button key={i} onClick={() => onAction?.(btn.url, btn.label)}
                    className="px-2.5 py-1.5 rounded-xl border text-[11px] font-medium transition-all cursor-pointer border-[#a3c8ed] bg-[#f4f8fd] text-[#1564b3] hover:bg-[#e8f1fb] hover:border-[#75ade4]">
                    {btn.label}
                </button>
            ))}
        </div>
        ...
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    ...
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#e8f1fb] transition-all bg-white border-gray-200 text-gray-800 placeholder-gray-400 focus:border-[#75ade4]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#1976d2] hover:bg-[#1564b3] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
