            ...
            <div className="p-3 space-y-1.5">
                ...
                {card.button && (
                    <button onClick={() => onAction?.(card.button.url, card.button.label)}
                        className="w-full mt-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-all bg-[#e6ebf0] text-[#002952] hover:bg-[#b3c2d1] flex items-center justify-center gap-1">
        ...
        <div className="flex flex-wrap gap-1.5">
            {buttons.map((btn, i) => (
                <button key={i} onClick={() => onAction?.(btn.url, btn.label)}
                    className="px-2.5 py-1.5 rounded-xl border text-[11px] font-medium transition-all cursor-pointer border-[#99adc2] bg-[#e6ebf0] text-[#002952] hover:bg-[#b3c2d1] hover:border-[#6685a3]">
                    {btn.label}
                </button>
            ))}
        </div>
        ...
        <motion.form initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
            ...
            {fields.map((f) => (
                <input key={f.key} type={f.key === 'email' ? 'email' : f.key === 'phone' ? 'tel' : 'text'}
                    ...
                    className="w-full px-3 py-2 rounded-xl border text-[12px] focus:outline-none focus:ring-1 focus:ring-[#e6ebf0] transition-all bg-white border-gray-200 text-gray-800 placeholder-gray-400 focus:border-[#6685a3]"
                />
            ))}
            <button type="submit"
                className="w-full py-2 rounded-xl text-[12px] font-semibold text-white bg-[#003366] hover:bg-[#002952] transition-all shadow-sm">
                {submitLabel}
            </button>
        </motion.form>
    );
