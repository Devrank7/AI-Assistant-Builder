...
export function Widget({ config }) {
    ...
    return (
        <div className={`fixed z-50 flex flex-col gap-4 antialiased ${positionClasses}`} style={{ fontFamily: "\'Poppins\', -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, sans-serif", ...dragStyle }}>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 scrollbar-hide bg-gradient-to-b from-gray-50/80 to-white" aria-live="polite">
                            ...
                            {isTyping && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 items-center ml-10 py-2">
                                    {[0, 1, 2].map((i) => (
                                        <motion.span key={i} className="w-2 h-2 bg-[#d2a11a] rounded-full"
            ...
            <motion.button
                ...
                className="w-[58px] h-[58px] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#d2a11a]/30 bg-gradient-to-br from-[#0f0335] via-[#1a0a45] to-[#120638] border border-white/10"
