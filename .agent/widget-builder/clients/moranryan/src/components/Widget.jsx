...
export function Widget({ config }) {
    ...
    const chatContent = (
        <>
            {/* Mobile swipe handle */}
            {isMobile && (
                <div className="flex justify-center pt-2 pb-0.5 cursor-grab active:cursor-grabbing bg-[#06081b]"
                    onTouchStart={handleSwipeStart} onTouchMove={handleSwipeMove} onTouchEnd={handleSwipeEnd}>
                    <div className="w-10 h-1 rounded-full bg-[#19193e]" />
                </div>
            )}

            {/* HEADER */}
            <div className="relative px-5 py-4 flex items-center justify-between ">
                <div className="absolute inset-0 bg-gradient-to-br from-[#7a00df] via-[#8600f5] to-[#007cba]" />
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#d7b3f5] border-[2.5px] border-[#7a00df] shadow-sm" />
                        )}
                    </div>
                    <div>
                ...
                <div className="relative flex items-center gap-1">
                    <div className="relative" ref={menuRef}>
                        ...
                        {showMenu && (
                            <div className="absolute right-0 top-full mt-1.5 w-[168px] rounded-2xl shadow-xl border overflow-hidden z-50 bg-[#0f0d29]/95 border-[#19193e]" style={{ backdropFilter: 'blur(16px)' }}>
                                <button onClick={() => { clearMessages(); setShowMenu(false); }}
                                    className="w-full px-3.5 py-2.5 text-left text-[12px] font-medium flex items-center gap-2.5 transition-colors text-[#e2e8f0] hover:bg-[#070a1f]">
                                ...
                                <button onClick={() => { cycleFontSize(); }}
                                    className="w-full px-3.5 py-2.5 text-left text-[12px] font-medium flex items-center justify-between transition-colors text-[#e2e8f0] hover:bg-[#070a1f]">
            ...
            {contacts && Object.keys(contacts).length > 0 && (
                <div className="flex items-center gap-1.5 px-4 py-1.5 border-b overflow-x-auto scrollbar-hide bg-[#0f0d29]/80 border-[#19193e]">
                    {contacts.phone && (
                        <a href={'tel:' + contacts.phone} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#070a1f]" target="_blank" rel="noopener">
                            <Phone size={12} /> {uiStrings.call}
                        </a>
                    )}
                    {contacts.email && (
                        <a href={'mailto:' + contacts.email} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#070a1f]">
                            <Mail size={12} /> Email
                        </a>
                    )}
                    {contacts.website && (
                        <a href={contacts.website} target="_blank" rel="noopener" className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#070a1f]">
            ...
            {pageTitle && !contextDismissed && messages.length === 0 && (
                <div className="flex items-center gap-2 px-4 py-2 border-b bg-[#7a00df]/10 border-[#19193e] text-[#e2e8f0]">
                    <Globe size={13} className="text-[#7a00df]" />
            ...
            <div ref={chatContainerRef} onScroll={handleChatScroll}
                    ...
                    <div key={idx}>
                        {shouldShowSeparator(idx) && msg.timestamp && (
                            <div className="flex items-center gap-3 my-3">
                                <div className="flex-1 h-px bg-[#19193e]/50" />
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#19193e] bg-[#0f0d29] text-[#e2e8f0] hover:bg-[#070a1f] hover:border-[#af66ec]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
                ...
                {isTyping && (
                    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-2 py-2">
                        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#f2e6fc] to-[#d7b3f5] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#ca99f2]/50">
                            <Sparkles size={13} className="text-[#7a00df]" />
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-[11px] font-medium text-[#64748b] ml-1">{config.botName || 'AI'} {uiStrings.isTyping}</span>
                            <div className="bg-[#0f0d29] border border-[#19193e] rounded-2xl rounded-tl-md px-4 py-3 flex items-center gap-1.5">
                ...
                {hasNewMessages && !isAtBottom && (
                    <div className="sticky bottom-2 left-0 right-0 flex justify-center z-10">
                        <button onClick={scrollToBottom}
                            className="new-msg-pill px-3.5 py-1.5 rounded-full text-[12px] font-semibold shadow-lg flex items-center gap-1.5 cursor-pointer transition-all hover:shadow-xl bg-[#0f0d29] text-[#e2e8f0] border border-[#19193e] shadow-black/30">
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 overflow-hidden bg-[#06081b] border-[#19193e]">
                        <div className="relative inline-block my-2.5">
                            <img src={selectedImage.previewUrl} alt="" className="h-16 w-auto rounded-xl border border-[#19193e] object-cover shadow-sm" />
            ...
            {/* INPUT */}
            <div className="px-4 pt-2 pb-3 border-t border-[#19193e] bg-[#06081b] space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#af66ec] bg-[#f2e6fc] text-[#7a00df] shadow-sm' : 'border-[#19193e] text-[#64748b] hover:text-[#7a00df] hover:border-[#af66ec] hover:bg-[#f2e6fc]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#af66ec] bg-[#f2e6fc] text-[#7a00df] shadow-sm animate-pulse' : 'border-[#19193e] text-[#64748b] hover:text-[#7a00df] hover:border-[#af66ec] hover:bg-[#f2e6fc]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-[#070a1f] text-[#e2e8f0] placeholder-[#64748b] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#19193e] focus:outline-none focus:border-[#af66ec] focus:ring-2 focus:ring-[#f2e6fc] focus:bg-[#101030] transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#7a00df] text-white flex items-center justify-center hover:bg-[#6200b2] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#7a00df]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
            ...
            {/* POWERED BY */}
            <div className="flex justify-center py-1.5 bg-[#06081b]">
    ...
    return (
        <>
            ...
            <div className={`fixed z-[9999] flex flex-col gap-3 antialiased ${positionClasses}`} style={{ fontFamily: "\'Roboto%3Awght%40100%3B300%3B400%3B500%3B700%3B900\', -apple-system, sans-serif", ...(isMobile && isOpen ? {} : dragStyle) }}>
                {/* Chat panel */}
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                            ...
                            className={isMobile
                                ? 'fixed inset-x-0 bottom-0 flex flex-col bg-[#06081b] shadow-2xl shadow-black/40 rounded-t-3xl overflow-hidden'
                                : 'relative w-[85vw] max-w-[360px] h-[60vh] max-h-[520px] sm:w-[360px] sm:h-[520px] rounded-3xl overflow-hidden flex flex-col bg-[#06081b] shadow-2xl shadow-black/40 border border-[#19193e]'}
                ...
                <AnimatePresence>
                    {showNudge && !isOpen && nudgeMessage && (
                        <motion.div key="nudge"
                            ...
                            className="max-w-[200px] sm:max-w-[220px] px-3.5 py-2.5 rounded-2xl shadow-xl shadow-black/30 border cursor-pointer relative bg-[#0f0d29] border-[#19193e] text-[#e2e8f0]"
                            onClick={() => { dismissNudge(); setIsOpen(true); }}
                        >
                            <button onClick={(e) => { e.stopPropagation(); dismissNudge(); }}
                                className="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full flex items-center justify-center transition-colors bg-[#19193e] text-[#64748b] hover:bg-[#070a1f]">
                                <X size={11} />
                            </button>
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#7a00df] to-[#007cba] animate-pulse-ring" />
                        )}
                        <motion.button
                            whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(122, 0, 223, 0.35)' }}
                            ...
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#7a00df]/30 bg-gradient-to-br from-[#7a00df] via-[#8600f5] to-[#007cba] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
                        ...
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center border-2 border-[#06081b] shadow-sm pointer-events-none">
                                {unreadCount}
                            </span>
                        )}
                    </div>
