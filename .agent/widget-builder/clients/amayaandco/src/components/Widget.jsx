...
export function Widget({ config }) {
    ...
    const chatContent = (
        <>
            ...
            <div className="relative px-5 py-4 flex items-center justify-between ">
                <div className="absolute inset-0 bg-gradient-to-br from-[#c9a96e] via-[#ceb27d] to-[#1a1a1a]" />
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#efe5d4] border-[2.5px] border-[#c9a96e] shadow-sm" />
                        )}
                    </div>
                    <div>
            ...
            <div ref={chatContainerRef} onScroll={handleChatScroll}
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#1e2d3d] bg-[#111927] text-[#e2e8f0] hover:bg-[#0f1720] hover:border-[#c9a96e]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
                ...
                {isTyping && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-start gap-2 py-2">
                        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#1e1911] to-[#322a1c] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#50442c]/50">
            ...
            <div className="px-4 pt-2 pb-3 border-t border-[#1e2d3d] bg-[#0b1018] space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#dfcba8] bg-[#14110b] text-[#c9a96e] shadow-sm' : 'border-[#1e2d3d] text-[#64748b] hover:text-[#c9a96e] hover:border-[#dfcba8] hover:bg-[#14110b]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#dfcba8] bg-[#14110b] text-[#c9a96e] shadow-sm animate-pulse' : 'border-[#1e2d3d] text-[#64748b] hover:text-[#c9a96e] hover:border-[#dfcba8] hover:bg-[#14110b]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-[#0f1720] text-[#e2e8f0] placeholder-[#64748b] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#1e2d3d] focus:outline-none focus:border-[#dfcba8] focus:ring-2 focus:ring-[#1e1911] focus:bg-[#162232] transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#c9a96e] text-white flex items-center justify-center hover:bg-[#a18758] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#c9a96e]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
    ...
    return (
        <>
            ...
            <div className={`fixed z-[9999] flex flex-col gap-3 antialiased ${positionClasses}`} style={{ fontFamily: "\'Inter\', sans-serif", ...(isMobile && isOpen ? {} : dragStyle) }}>
                {/* Chat panel */}
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#c9a96e] to-[#1a1a1a] animate-pulse-ring" />
                        )}
                        <motion.button
                            whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(201, 169, 110, 0.35)' }}
                            ...
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#c9a96e]/30 bg-gradient-to-br from-[#c9a96e] via-[#ceb27d] to-[#1a1a1a] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
