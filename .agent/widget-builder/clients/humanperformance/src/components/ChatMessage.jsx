...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick, onSpeak, isSpeaking }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col max-w-[85%] sm:max-w-[78%]">
                {imageUrl && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`mb-1.5 ${isBot ? '' : 'flex justify-end'}`}>
                        <div className="relative group/img cursor-pointer overflow-hidden rounded-2xl border border-[#27343e] shadow-sm" onClick={() => onImageClick?.(imageUrl)}>
                ...
                {content && (
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ? 'bg-[#181f29] text-[#e2e8f0] border border-[#27343e] shadow-sm rounded-bl-md'
                              : 'bg-gradient-to-r from-[#f2dede] to-[#a94442] text-white rounded-br-md shadow-sm'
                    }`}>
                        <div className="max-w-none msg-text [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className="underline decoration-1 underline-offset-2 transition-colors text-[#f2dede] hover:text-[#c2b2b2]">{children}</a>
                                    ),
                                    strong: ({ children }) => (
                ...
                <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                    ...
                    {isBot && !isError && content && (
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-[#475569] hover:text-[#f2dede] transition-all duration-200" aria-label="Copy">
                            {copied ? <Check size={11} className="text-[#f2dede]" /> : <Copy size={11} />}
                        </button>
                    )}
                    {isBot && !isError && content && onSpeak && (
                        <button onClick={() => onSpeak(content)}
                            className={`p-0.5 transition-all duration-200 ${isSpeaking ? 'text-[#f2dede] opacity-100' : 'opacity-0 group-hover:opacity-100 text-[#475569] hover:text-[#f2dede]'}`}
            ...
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#fefcfc] to-[#fbf5f5] border border-[#faf2f2]/50 text-[#f2dede] flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
