    ...
    const chatContent = (
        <>
            ...
            <div className="relative px-5 py-4 flex items-center justify-between ">
                <div className="absolute inset-0 bg-gradient-to-br from-[#16a085] via-[#2daa91] to-[#2c3e50]" />
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#b9e3da] border-[2.5px] border-[#16a085] shadow-sm" />
                        )}
                    </div>
                    <div>
            ...
            {contacts && Object.keys(contacts).length > 0 && (
                <div className="flex items-center gap-1.5 px-4 py-1.5 border-b overflow-x-auto scrollbar-hide bg-gray-50/60 border-gray-100">
                    {contacts.phone && (
                        <a href={'tel:' + contacts.phone} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-gray-500 hover:text-[#16a085] hover:bg-white" target="_blank" rel="noopener">
                            <Phone size={12} /> {uiStrings.call}
                        </a>
                    )}
                    {contacts.email && (
                        <a href={'mailto:' + contacts.email} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-gray-500 hover:text-[#16a085] hover:bg-white">
                            <Mail size={12} /> Email
                        </a>
                    )}
                    {contacts.website && (
                        <a href={contacts.website} target="_blank" rel="noopener" className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-gray-500 hover:text-[#16a085] hover:bg-white">
            ...
            <div ref={chatContainerRef} onScroll={handleChatScroll}
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#a2d9ce] bg-[#e8f6f3] text-[#12806a] hover:bg-[#b9e3da] hover:border-[#73c6b6]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
                ...
                {isTyping && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-start gap-2 py-2">
                        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#e8f6f3] to-[#b9e3da] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#a2d9ce]/50">
                            <Sparkles size={13} className="text-[#16a085]" />
                        </div>
            ...
            <div className="px-4 pt-2 pb-3 border-t border-gray-100 bg-white space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#73c6b6] bg-[#e8f6f3] text-[#16a085] shadow-sm' : 'border-gray-200 text-gray-400 hover:text-[#16a085] hover:border-[#73c6b6] hover:bg-[#e8f6f3]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#73c6b6] bg-[#e8f6f3] text-[#16a085] shadow-sm animate-pulse' : 'border-gray-200 text-gray-400 hover:text-[#16a085] hover:border-[#73c6b6] hover:bg-[#e8f6f3]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-gray-50/80 text-gray-800 placeholder-gray-400 rounded-xl py-2.5 pl-3.5 pr-3.5 border border-gray-200 focus:outline-none focus:border-[#73c6b6] focus:ring-2 focus:ring-[#e8f6f3] focus:bg-white transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#16a085] text-white flex items-center justify-center hover:bg-[#12806a] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#16a085]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#16a085] to-[#2c3e50] animate-pulse-ring" />
                        )}
                        <motion.button
                            whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(22, 160, 133, 0.35)' }}
                            ...
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#16a085]/30 bg-gradient-to-br from-[#16a085] via-[#2daa91] to-[#2c3e50] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
