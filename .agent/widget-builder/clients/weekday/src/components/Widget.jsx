...
export function Widget({ config }) {
    ...
    const chatContent = (
        <>
            {/* Mobile swipe handle */}
            {isMobile && (
                <div className="flex justify-center pt-2 pb-0.5 cursor-grab active:cursor-grabbing bg-[#05111b]"
                    onTouchStart={handleSwipeStart} onTouchMove={handleSwipeMove} onTouchEnd={handleSwipeEnd}>
                    <div className="w-10 h-1 rounded-full bg-[#152f3d]" />
                </div>
            )}

            {/* HEADER */}
            <div className="relative px-5 py-4 flex items-center justify-between ">
                <div className="absolute inset-0 bg-gradient-to-br from-[#5bbad5] via-[#64cdea] to-[#186750]" />
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#ceeaf2] border-[2.5px] border-[#5bbad5] shadow-sm" />
                        )}
                    </div>
                    <div>
                ...
                <div className="relative flex items-center gap-1">
                    <div className="relative" ref={menuRef}>
                        ...
                        {showMenu && (
                            <div className="absolute right-0 top-full mt-1.5 w-[168px] rounded-2xl shadow-xl border overflow-hidden z-50 bg-[#0c1c28]/95 border-[#152f3d]" style={{ backdropFilter: 'blur(16px)' }}>
                                <button onClick={() => { clearMessages(); setShowMenu(false); }}
                                    className="w-full px-3.5 py-2.5 text-left text-[12px] font-medium flex items-center gap-2.5 transition-colors text-[#e2e8f0] hover:bg-[#05151f]">
                                ...
                                <button onClick={() => { cycleFontSize(); }}
                                    className="w-full px-3.5 py-2.5 text-left text-[12px] font-medium flex items-center justify-between transition-colors text-[#e2e8f0] hover:bg-[#05151f]">
            ...
            {contacts && Object.keys(contacts).length > 0 && (
                <div className="flex items-center gap-1.5 px-4 py-1.5 border-b overflow-x-auto scrollbar-hide bg-[#0c1c28]/80 border-[#152f3d]">
                    {contacts.phone && (
                        <a href={'tel:' + contacts.phone} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#05151f]" target="_blank" rel="noopener">
                            <Phone size={12} /> {uiStrings.call}
                        </a>
                    )}
                    {contacts.email && (
                        <a href={'mailto:' + contacts.email} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#05151f]">
                            <Mail size={12} /> Email
                        </a>
                    )}
                    {contacts.website && (
                        <a href={contacts.website} target="_blank" rel="noopener" className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[12px] font-medium transition-all whitespace-nowrap text-[#64748b] hover:text-[#e2e8f0] hover:bg-[#05151f]">
            ...
            {pageTitle && !contextDismissed && messages.length === 0 && (
                <div className="flex items-center gap-2 px-4 py-2 border-b bg-[#5bbad5]/10 border-[#152f3d] text-[#e2e8f0]">
            ...
            <div ref={chatContainerRef} onScroll={handleChatScroll}
                    ...
                    <div key={idx}>
                        {shouldShowSeparator(idx) && msg.timestamp && (
                            <div className="flex items-center gap-3 my-3">
                                <div className="flex-1 h-px bg-[#152f3d]/50" />
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#152f3d] bg-[#0c1c28] text-[#e2e8f0] hover:bg-[#05151f] hover:border-[#9dd6e6]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
                ...
                {isTyping && (
                    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-2 py-2">
                        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#eff8fb] to-[#ceeaf2] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#bde3ee]/50">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="bg-[#0c1c28] border border-[#152f3d] rounded-2xl rounded-tl-md px-4 py-3 flex items-center gap-1.5">
                ...
                {hasNewMessages && !isAtBottom && (
                    <div className="sticky bottom-2 left-0 right-0 flex justify-center z-10">
                        <button onClick={scrollToBottom}
                            className="new-msg-pill px-3.5 py-1.5 rounded-full text-[12px] font-semibold shadow-lg flex items-center gap-1.5 cursor-pointer transition-all hover:shadow-xl bg-[#0c1c28] text-[#e2e8f0] border border-[#152f3d] shadow-black/30">
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 overflow-hidden bg-[#05111b] border-[#152f3d]">
                        <div className="relative inline-block my-2.5">
                            <img src={selectedImage.previewUrl} alt="" className="h-16 w-auto rounded-xl border border-[#152f3d] object-cover shadow-sm" />
            ...
            {/* INPUT */}
            <div className="px-4 pt-2 pb-3 border-t border-[#152f3d] bg-[#05111b] space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#9dd6e6] bg-[#eff8fb] text-[#5bbad5] shadow-sm' : 'border-[#152f3d] text-[#64748b] hover:text-[#5bbad5] hover:border-[#9dd6e6] hover:bg-[#eff8fb]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#9dd6e6] bg-[#eff8fb] text-[#5bbad5] shadow-sm animate-pulse' : 'border-[#152f3d] text-[#64748b] hover:text-[#5bbad5] hover:border-[#9dd6e6] hover:bg-[#eff8fb]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-[#05151f] text-[#e2e8f0] placeholder-[#64748b] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#152f3d] focus:outline-none focus:border-[#9dd6e6] focus:ring-2 focus:ring-[#eff8fb] focus:bg-[#0d212f] transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#5bbad5] text-white flex items-center justify-center hover:bg-[#4995aa] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#5bbad5]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
            ...
            {/* POWERED BY */}
            <div className="flex justify-center py-1.5 bg-[#05111b]">
    ...
    return (
        <>
            ...
            <div className={`fixed z-[9999] flex flex-col gap-3 antialiased ${positionClasses}`} style={{ fontFamily: "\'__Lato_33621c\', -apple-system, sans-serif", ...(isMobile && isOpen ? {} : dragStyle) }}>
                {/* Chat panel */}
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                            ...
                            className={isMobile
                                ? 'fixed inset-x-0 bottom-0 flex flex-col bg-[#05111b] shadow-2xl shadow-black/40 rounded-t-3xl overflow-hidden'
                                : 'relative w-[85vw] max-w-[360px] h-[60vh] max-h-[520px] sm:w-[360px] sm:h-[520px] rounded-3xl overflow-hidden flex flex-col bg-[#05111b] shadow-2xl shadow-black/40 border border-[#152f3d]'}
                ...
                <AnimatePresence>
                    {showNudge && !isOpen && nudgeMessage && (
                        <motion.div key="nudge"
                            ...
                            className="max-w-[200px] sm:max-w-[220px] px-3.5 py-2.5 rounded-2xl shadow-xl shadow-black/30 border cursor-pointer relative bg-[#0c1c28] border-[#152f3d] text-[#e2e8f0]"
                            onClick={() => { dismissNudge(); setIsOpen(true); }}
                        >
                            <button onClick={(e) => { e.stopPropagation(); dismissNudge(); }}
                                className="absolute -top-1.5 -right-1.5 w-6 h-6 rounded-full flex items-center justify-center transition-colors bg-[#152f3d] text-[#64748b] hover:bg-[#05151f]">
                                <X size={11} />
                            </button>
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#5bbad5] to-[#186750] animate-pulse-ring" />
                        )}
                        <motion.button
                            ...
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#5bbad5]/30 bg-gradient-to-br from-[#5bbad5] via-[#64cdea] to-[#186750] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
                        ...
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center border-2 border-[#05111b] shadow-sm pointer-events-none">
                                {unreadCount}
                            </span>
                        )}
                    </div>
