...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick }) {
    ...
    return (
        ...
        >
            {/* Bot Avatar */}
            {isBot && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#fce4e4] to-[#f8cccc] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#f09090]/50 overflow-hidden p-0.5">
                    {!logoErr ? (
                        <img src="https://www.google.com/s2/favicons?domain=www.g-tuning.com.ua&sz=32" alt="" className="w-full h-full object-contain rounded-sm" onError={() => setLogoErr(true)} />
                    ) : (
                        <Sparkles size={13} className="text-[#c01e1e]" />
                    )}
                </div>
            )}
            ...
            <div className="flex flex-col max-w-[78%]">
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ...
                              : 'bg-gradient-to-r from-[#fef0f0] to-[#fce4e4] text-gray-700 border border-[#f09090]/50 rounded-br-md shadow-sm'
                    }`}>
                        <div className="max-w-none [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className="underline decoration-1 underline-offset-2 transition-colors text-[#c01e1e] hover:text-[#a81a1a]">{children}</a>
                                    ),
                                    strong: ({ children }) => (
                ...
                <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                    ...
                    {isBot && !isError && content && (
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-[#D72323] transition-all duration-200" aria-label="Copy">
                            {copied ? <Check size={11} className="text-[#D72323]" /> : <Copy size={11} />}
                        </button>
                    )}
                    {isError && onRetry && (
            ...
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#fce4e4] to-[#f8cccc] border border-[#f09090]/50 text-[#c01e1e] flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
