...
Read leads from a Google Sheets spreadsheet (today's "DD.MM.YYYY Проверенные лиды"), filter leads that **already have a widget** (`hasWidget = TRUE`), visit each lead's website to analyze their business niche, and generate a personalized first outreach message based on the prompt templates in `.agent/prompts/`.
...
1. **ALWAYS use**: `.agent/prompts/outreach_agent_system_prompt.md` — the master sales prompt (Phase 1: OPENER section)
2. **If niche is `dental`**: Also reference `.agent/prompts/niche_dental_addon.md`
3. **If niche is `beauty_salon`**: Also reference `.agent/prompts/niche_beauty_salon_addon.md`
...
- `{BASE_URL}` = value of `NEXT_PUBLIC_BASE_URL` environment variable (default: `https://winbixai.com`)
...
```
https://winbixai.com/demo/client-website?client=smile-dental&website=https%3A%2F%2Fsmile-dental.com
```
...
- **DO** reference niche addon prompts (`.agent/prompts/niche_dental_addon.md`, `.agent/prompts/niche_beauty_salon_addon.md`) when the niche matches
