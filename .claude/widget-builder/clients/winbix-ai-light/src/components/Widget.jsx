            ...
            <div className="relative px-6 py-5 flex items-center justify-between ">
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#34D399] border-[2.5px] border-[#059669] shadow-sm" />
                        )}
                    </div>
                    <div>
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#DBEAFE] bg-[#EFF6FF] text-[#2563EB] hover:bg-[#DBEAFE] hover:border-[#3B82F6]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
            ...
            <div className="px-4 pt-2 pb-3 border-t border-gray-100 bg-white space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#3B82F6] bg-[#EFF6FF] text-[#2563EB] shadow-sm' : 'border-gray-200 text-gray-400 hover:text-[#2563EB] hover:border-[#2563EB] hover:bg-[#EFF6FF]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#3B82F6] bg-[#EFF6FF] text-[#2563EB] shadow-sm animate-pulse' : 'border-gray-200 text-gray-400 hover:text-[#2563EB] hover:border-[#2563EB] hover:bg-[#EFF6FF]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-gray-50/80 text-gray-800 placeholder-gray-400 rounded-xl py-2.5 pl-3.5 pr-3.5 border border-gray-200 focus:outline-none focus:border-[#3B82F6] focus:ring-2 focus:ring-[#FFFFFF] focus:bg-white transition-all resize-none text-[13.5px] leading-relaxed"
