...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick, onSpeak, isSpeaking }) {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col max-w-[78%]">
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ...
                              : 'bg-gradient-to-r from-[#FFF9F0] to-[#FEF5E8] text-gray-700 border border-[#F6D8A8]/50 rounded-br-md shadow-sm'
                    }`}>
                        <div className="max-w-none msg-text [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className="underline decoration-1 underline-offset-2 transition-colors text-[#b45f05] hover:text-[#9A5004]">{children}</a>
                                    ),
                                    strong: ({ children }) => (
                ...
                <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                    ...
                    {isBot && !isError && content && (
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-[#f6b26b] transition-all duration-200" aria-label="Copy">
                            {copied ? <Check size={11} className="text-[#f6b26b]" /> : <Copy size={11} />}
                        </button>
                    )}
                    {isBot && !isError && content && onSpeak && (
                        <button onClick={() => onSpeak(content)}
                            className={`p-0.5 transition-all duration-200 ${isSpeaking ? 'text-[#f6b26b] opacity-100' : 'opacity-0 group-hover:opacity-100 text-gray-300 hover:text-[#f6b26b]'}`}
            ...
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-[#FEF5E8] to-[#FDE8CC] border border-[#F6D8A8]/50 text-[#b45f05] flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
