...
export function Widget({ config }) {
    ...
    const chatContent = (
        <>
            ...
            {/* HEADER */}
            <div className="relative px-6 py-5 flex items-center justify-between backdrop-blur-md bg-white/20 border-b border-white/30 shadow-sm">
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#ff7f50] border-[2.5px] border-[#0f1117] shadow-sm" />
                        )}
                    </div>
                    <div>
            ...
            {pageTitle && !contextDismissed && messages.length === 0 && (
                <div className="flex items-center gap-2 px-4 py-2 border-b bg-[#1976d2]/10 border-[#2a2d35] text-[#ffffff]">
                            ...
                            <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                                ...
                                {msg.suggestions.map((s, si) => (
                                    <button key={si} onClick={() => { detectLang(s); sendMessage(s); }}
                                        className="px-2.5 py-1.5 rounded-lg border text-[12px] font-medium transition-all duration-200 cursor-pointer border-[#2a2d35] bg-[#1a1d23] text-[#ffffff] hover:bg-[#1e212b] hover:border-[#1976d2]">
                                        {s}
                                    </button>
                                ))}
                            </motion.div>
            ...
            <div className="px-4 pt-2 pb-3 border-t border-[#2a2d35] bg-[#0f1117] space-y-1.5 safe-area-bottom">
                ...
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#1976d2] bg-[#1e212b] text-[#1976d2] shadow-sm' : 'border-[#2a2d35] text-[#9ca3af] hover:text-[#0d47a1] hover:border-[#0d47a1] hover:bg-[#1565c0]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-[#1e212b] text-[#ffffff] placeholder-[#9ca3af] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#2a2d35] focus:outline-none focus:border-[#1976d2] focus:ring-2 focus:ring-[#1976d2] focus:bg-[#616161] transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#1976d2] text-white flex items-center justify-center hover:bg-[#0d47a1] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#1976d2]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#1976d2] to-[#0d47a1] animate-pulse-ring" />
                        )}
                        <motion.button
                            ...
                            className={`w-[58px] h-[58px] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#1976d2]/30 bg-gradient-to-br from-[#1976d2] via-[#1565c0] to-[#0d47a1] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
