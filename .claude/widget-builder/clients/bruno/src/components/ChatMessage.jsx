...
function ChatMessage({ role, content, timestamp, isError, onRetry, imageUrl, onImageClick }) {
    ...
    return (
        ...
        >
            {/* Bot Avatar */}
            {isBot && (
                <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-green-100 to-emerald-100 flex items-center justify-center flex-shrink-0 shadow-sm border border-green-200/50">
            ...
            <div className="flex flex-col max-w-[78%]">
                    ...
                    <div className={`rounded-2xl px-3.5 py-2.5 text-[13px] leading-[1.6] ${
                        isError
                            ...
                            : isBot
                              ...
                              : 'bg-gradient-to-br from-green-500 to-emerald-600 text-white rounded-br-md shadow-sm shadow-green-500/20'
                    }`}>
                        <div className="max-w-none [&>p]:my-0 [&>p+p]:mt-2 [&>ul]:my-1.5 [&>ol]:my-1.5 [&>ul]:pl-4 [&>ol]:pl-4 [&>ul]:list-disc [&>ol]:list-decimal">
                            <ReactMarkdown
                                components={{
                                    a: ({ href, children }) => (
                                        <a href={href} target="_blank" rel="noopener noreferrer" className={`underline decoration-1 underline-offset-2 transition-colors ${isBot ? 'text-green-600 hover:text-green-700' : 'text-white/90 hover:text-white'}`}>{children}</a>
                                    ),
                                    strong: ({ children }) => (
                                        <strong className={`font-semibold ${isBot ? 'text-gray-900' : 'text-white'}`}>{children}</strong>
                                    ),
                                    li: ({ children }) => (
                ...
                <div className={`flex items-center gap-2 mt-1 px-1 ${isBot ? '' : 'justify-end'}`}>
                    ...
                    {isBot && !isError && content && (
                        <button onClick={handleCopy} className="opacity-0 group-hover:opacity-100 p-0.5 text-gray-300 hover:text-green-500 transition-all duration-200" aria-label="Copy">
            ...
            {/* User Avatar */}
            {!isBot && (
                <div className="w-7 h-7 rounded-xl flex items-center justify-center bg-gradient-to-br from-green-100 to-emerald-100 border border-green-200/50 text-green-600 flex-shrink-0 shadow-sm">
                    <User size={13} />
                </div>
            )}
        </motion.div>
