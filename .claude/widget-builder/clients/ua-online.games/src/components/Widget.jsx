    ...
    const chatContent = (
        <>
            ...
            <div className="relative px-6 py-5 flex items-center justify-between ">
                <div className="absolute inset-0 bg-gradient-to-br from-[#1a1d23] via-[#0f1117] to-[#000000]" />
                ...
                <div className="relative flex items-center gap-3">
                    <div className="relative">
                        ...
                        {!isOffline && (
                            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-[#10b981] border-[2.5px] border-[#0f1117] shadow-sm" />
                        )}
                    </div>
                    <div>
            ...
            {pageTitle && !contextDismissed && messages.length === 0 && (
                <div className="flex items-center gap-2 px-4 py-2 border-b bg-[#4ade80]/10 border-[#2a2d35] text-[#ffffff]">
            ...
            <div ref={chatContainerRef} onScroll={handleChatScroll}
                ...
                {isTyping && (
                    <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-2 py-2">
                        <div className="w-7 h-7 rounded-xl bg-gradient-to-br from-[#1e293b] to-[#0f172a] flex items-center justify-center flex-shrink-0 shadow-sm border border-[#2a2d35]/50">
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 overflow-hidden bg-[#0f1117] border-[#2a2d35]">
                        <div className="relative inline-block my-2.5">
                            <img src={selectedImage.previewUrl} alt="" className="h-16 w-auto rounded-xl border border-[#2a2d35] object-cover shadow-sm" />
            ...
            <div className="px-4 pt-2 pb-3 border-t border-[#2a2d35] bg-[#0f1117] space-y-1.5 safe-area-bottom">
                <form onSubmit={handleSubmit} className="flex items-end gap-2">
                    ...
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                        className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#4ade80] bg-[#1a1d23] text-[#ffffff] shadow-sm' : 'border-[#2a2d35] text-[#9ca3af] hover:text-[#ffffff] hover:border-[#22c55e] hover:bg-[#1a1d23]'}`}
                        aria-label="Upload photo">
                        <ImagePlus size={16} />
                    </button>
                    {voiceSupported && config.features?.voiceInput !== false && (
                        <button type="button" onClick={handleVoiceToggle}
                            className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${isListening ? 'border-[#4ade80] bg-[#1a1d23] text-[#ffffff] shadow-sm animate-pulse' : 'border-[#2a2d35] text-[#9ca3af] hover:text-[#ffffff] hover:border-[#22c55e] hover:bg-[#1a1d23]'}`}
                    ...
                    <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                        ...
                        rows={1}
                        className="flex-1 min-w-0 bg-[#1e212b] text-[#ffffff] placeholder-[#9ca3af] rounded-xl py-2.5 pl-3.5 pr-3.5 border border-[#2a2d35] focus:outline-none focus:border-[#4ade80] focus:ring-2 focus:ring-[#1e293b] focus:bg-[#1a1d23] transition-all resize-none text-[13.5px] leading-relaxed"
                        style={{ maxHeight: '100px' }}
                    />
                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                        className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#4ade80] text-white flex items-center justify-center hover:bg-[#22c55e] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#4ade80]/25">
                        <Send size={16} />
                    </button>
                </form>
            </div>
                ...
                {(!isMobile || !isOpen) && (
                    <div className="relative self-end">
                        {!isOpen && unreadCount > 0 && (
                            <span className="absolute inset-0 rounded-2xl bg-gradient-to-br from-[#4ade80] to-[#16a34a] animate-pulse-ring" />
                        )}
                        <motion.button
                            whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(74, 222, 128, 0.35)' }}
                            ...
                            className={`w-[58px] h-[58px] rounded-2xl flex items-center justify-center text-white shadow-lg shadow-[#000000]/30 bg-gradient-to-br from-[#4ade80] via-[#22c55e] to-[#16a34a] border border-white/10 ${!isOpen ? 'animate-breathe' : ''}`}
