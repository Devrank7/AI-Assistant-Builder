...
export function Widget({ config }) {
    ...
    return (
        <div className={`fixed z-50 flex flex-col gap-4 antialiased ${positionClasses}`} style={{ fontFamily: "\'Plus Jakarta Sans\', -apple-system, BlinkMacSystemFont, sans-serif", ...dragStyle }}>
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="relative w-[85vw] max-w-[380px] h-[60vh] max-h-[560px] sm:w-[380px] sm:h-[560px] rounded-3xl overflow-hidden flex flex-col bg-white shadow-2xl shadow-black/15 border border-gray-100"
                        role="dialog"
                        aria-label="Chat widget"
                    >
                        {/* HEADER */}
                        <div className="relative px-6 py-5 flex items-center justify-between overflow-hidden border-b border-[#C1A01E]/20">
                            <div className="absolute inset-0 bg-gradient-to-br from-[#252A2F] via-[#2d3238] to-[#353b42]" />
                            ...
                            <div className="relative flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-[15px] text-white tracking-tight leading-tight">{config.bot.name}</h3>
                        ...
                        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 scrollbar-hide bg-gradient-to-b from-gray-50/80 to-white" aria-live="polite">
                            ...
                            {isTyping && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 items-center ml-10 py-2">
                                    {[0, 1, 2].map((i) => (
                                        <motion.span key={i} className="w-2 h-2 bg-[#C1A01E] rounded-full"
                        ...
                        <div className="px-4 py-3.5 border-t border-gray-100 bg-white space-y-2.5">
                            ...
                            <form onSubmit={handleSubmit} className="flex items-end gap-2">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className={`flex-shrink-0 p-2.5 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#e6d98a] bg-[#fdf9eb] text-[#a88d1a] shadow-sm' : 'border-gray-200 text-gray-400 hover:text-[#a88d1a] hover:border-[#e6d98a] hover:bg-[#fdf9eb]'}`}
                                ...
                                <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                                    ...
                                    rows={1}
                                    className="flex-1 min-w-0 bg-gray-50/80 text-gray-800 placeholder-gray-400 rounded-xl py-2.5 pl-3.5 pr-3.5 border border-gray-200 focus:outline-none focus:border-[#C1A01E] focus:ring-2 focus:ring-[#faf6e3] focus:bg-white transition-all resize-none text-[13.5px] leading-relaxed"
                                    style={{ maxHeight: '100px' }}
                                />
                                <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                                    className="flex-shrink-0 w-9 h-9 rounded-xl bg-[#C1A01E] text-white flex items-center justify-center hover:bg-[#a88d1a] active:scale-95 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed shadow-md shadow-[#C1A01E]/25">
                                    <Send size={16} />
                                </button>
                            </form>
                        </div>
            ...
            {/* TOGGLE BUTTON */}
            <motion.button
                whileHover={isDragging ? {} : { scale: 1.08, boxShadow: '0 8px 30px rgba(193, 160, 30, 0.35)' }}
                ...
                className="w-16 h-16 rounded-[20px] flex items-center justify-center text-white shadow-lg shadow-[#C1A01E]/30 bg-gradient-to-br from-[#C1A01E] via-[#d4b42e] to-[#a88d1a] border border-white/10"
