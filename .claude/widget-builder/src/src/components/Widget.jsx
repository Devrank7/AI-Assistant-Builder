            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="relative w-[85vw] max-w-[360px] h-[55vh] max-h-[480px] sm:w-[360px] sm:h-[480px] rounded-3xl overflow-hidden flex flex-col bg-white shadow-2xl shadow-black/15 border border-gray-100"
                        role="dialog"
                            ...
                            <div className="relative flex items-center gap-3">
                                <div className="relative">
                                    <div className="w-10 h-10 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-inner shadow-white/10 overflow-hidden p-1.5">
                                        {!headerLogoErr ? (
                                            <img src="https://www.google.com/s2/favicons?domain=www.zdorovo-dent.com.ua&sz=64" alt="" className="w-full h-full object-contain rounded" crossOrigin="anonymous" onError={() => setHeaderLogoErr(true)} />
                                        ) : (
                        ...
                        <div className="px-4 py-3.5 border-t border-gray-100 bg-white space-y-2.5">
                            ...
                            <form onSubmit={handleSubmit} className="flex items-center gap-2">
                                ...
                                <button type="button" onClick={() => fileInputRef.current?.click()}
                                    className={`flex-shrink-0 p-2 rounded-xl border transition-all duration-200 ${selectedImage ? 'border-[#7ee8c0] bg-[#e6faf2] text-[#00b070] shadow-sm' : 'border-gray-200 text-gray-400 hover:text-[#00b070] hover:border-[#7ee8c0] hover:bg-[#e6faf2]'}`}
                                ...
                                <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown}
                                        ...
                                        rows={1}
                                        className="flex-1 bg-gray-50/80 text-gray-800 placeholder-gray-400 rounded-xl py-2.5 pl-3.5 pr-3.5 border border-gray-200 focus:outline-none focus:border-[#00d084] focus:ring-2 focus:ring-[#ccf5e5] focus:bg-white transition-all resize-none text-[13.5px] leading-relaxed"
                                        style={{ maxHeight: '100px' }}
                                    />
                                    <button type="submit" disabled={(!inputValue.trim() && !selectedImage) || isLoading}
                                        className="flex-shrink-0 p-2.5 rounded-xl bg-gradient-to-br from-[#00d084] to-[#00b070]  text-white hover:from-[#00b070] hover:to-[#009960] active:scale-95 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed shadow-sm">
                                        <Send size={14} />
                                    </button>
                            </form>
                        </div>
