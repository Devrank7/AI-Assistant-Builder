...
export class CryptomusProvider implements PaymentProvider {
    ...
    async getSubscriptionStatus(subscriptionId: string): Promise<SubscriptionStatus> {
        try {
            const response = await this.request<{ result: { status: string; last_pay_off?: string } }>(
                '/recurrence/info',
                { uuid: subscriptionId }
            );
            ...
            return {
                ...
                status: status === 'active' ? 'active' :
                    status === 'wait' ? 'pending' :
                        status === 'cancel' ? 'canceled' : 'past_due',
                lastPaymentDate: response.result.last_pay_off
                    ? new Date(response.result.last_pay_off)
                    : undefined,
            };
        } catch {
            return {
    ...
    async handleWebhook(payload: unknown, signature?: string): Promise<WebhookResult> {
        try {
            ...
            const additionalData = data.additional_data
                ? JSON.parse(data.additional_data as string)
                : {};
...
export function createCryptomusProvider(): CryptomusProvider | null {
    ...
    const webhookUrl = process.env.CRYPTOMUS_WEBHOOK_URL || `${process.env.NEXT_PUBLIC_BASE_URL}/api/payments/webhook/cryptomus`;
