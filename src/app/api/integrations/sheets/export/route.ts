...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!clientId || !spreadsheetId) {
      return NextResponse.json({ success: false, error: 'clientId and spreadsheetId are required' }, { status: 400 });
    }

    if (!isConfigured()) {
      return NextResponse.json({ success: false, error: 'Google Sheets not configured' }, { status: 400 });
    }
