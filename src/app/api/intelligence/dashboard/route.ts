...
import { getDashboardData } from '@/lib/conversationIntelligence';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getDashboardData(clientId, days);
    return successResponse(data);
  } catch (err) {
    console.error('[intelligence/dashboard] Error:', err);
    return Errors.internal('Failed to load dashboard data');
  }
}
